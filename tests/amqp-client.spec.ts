...
describe('When a connection fails', () => {
  ...
  it(
    'should stop reconnecting after reaching the maximum number reconnection of attempts',
    async () => {
      connectMock.mockRejectedValue(new Error('Connection failed'));
      const client = generateClient();
      await expect(client.sendMessage('test-queue', { key: 'value' })).rejects.toThrow(
